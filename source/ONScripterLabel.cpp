...
void ONScripterLabel::initSDL()
{
    ...
    if ( SDL_Init( SDL_INIT_EVENTS ) < 0 ) {
        fprintf( stderr, "Couldn't initialize SDL_events; %s\n", SDL_GetError() );
    }
#endif
...
#ifdef __SWITCH__
    screen_width_wide = (screen_height * 16) / 9;     // Fix widescreen view
    bar_length[0] = bar_length[1] = (screen_width_wide - screen_width) / 2;
    if ( (screen_width_wide - screen_width) % 2 )
        bar_length[0]++;
#endif
    ...
    window = SDL_CreateWindow("ONScripter-SDL2",
                        ...
                        #ifdef __SWITCH__
                              screen_width_wide, screen_height,
                        #else
...
#ifdef __SWITCH__
    Uint32 black;
    black = SDL_MapRGB( screen_surface->format, 0, 0, 0 );

    cinematic_bars[0].x = 0;
    cinematic_bars[0].y = 0;
    cinematic_bars[0].w = bar_length[0];
    cinematic_bars[0].h = screen_height;

    cinematic_bars[1].x = screen_width + bar_length[0];
    cinematic_bars[1].y = 0;
    cinematic_bars[1].w = bar_length[1];
    cinematic_bars[1].h = screen_height;

    SDL_FillRects( screen_surface, cinematic_bars, 2, black );
#endif
...
int ONScripterLabel::init()
{
    ...
    if ( switch_check_files() < 0 )
        switch_nofile_splash();
#endif
...
#ifdef __SWITCH__
    screen_clip.x = rect.x + bar_length[0];
    screen_clip.y = rect.y;
    screen_clip.w = rect.w;
    screen_clip.h = rect.h;

    if ( SDL_BlitSurface( accumulation_surface, &rect, screen_surface, &screen_clip ) != 0 )
        SDL_Quit();
#else
...
#if defined(__SWITCH__)
    SDL_UpdateWindowSurfaceRects( window, &screen_clip, 1 );
#elif defined(SDL2)
...
#ifdef SDL2
 #ifdef __SWITCH__
SDL_Rect *screen_clips;
 #endif

void ONScripterLabel::fastFlushDirect( SDL_Rect rects[], int num, int refresh_mode )
{
 #ifdef DEBUG
    // clock_gettime( CLOCK_MONOTONIC, &start );
    for ( int i=0; i < num; i++ )
 ...
 #ifdef __SWITCH__
        screen_clips[ i ].x = rects[ i ].x + bar_length[0];
        screen_clips[ i ].y = rects[ i ].y;
        screen_clips[ i ].w = rects[ i ].w;
        screen_clips[ i ].h = rects[ i ].h;

        if ( SDL_BlitSurface( accumulation_surface, &rects[i], screen_surface, &screen_clips[i] ) != 0 )
            SDL_Quit();
 #else
 ...
 #if defined(__SWITCH__)
    SDL_UpdateWindowSurfaceRects( window, screen_clips, num );
    delete[] screen_clips;
 #else
    SDL_UpdateWindowSurfaceRects( window, rects, num );
 #endif
